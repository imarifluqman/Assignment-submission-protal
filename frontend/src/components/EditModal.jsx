...
const EditModal = ({func}) => {
    const  {instructorId,assignmentId} = useParams()
    let { open , setOpen,handleGetAssignment ,assignment } = func;
  ...
  const handleSubmit= async()=>{
    if(!AssignmentData.title||!AssignmentData.course||!AssignmentData.description|| !AssignmentData.dueDate){
      console.log("Err");
      setErrMsg(true)
    }else{
      setErrMsg(false)
      try {
        const response = await  axios.put(`http://localhost:5000/api/assignments/${assignmentId}`,{
        ...
        if(response){
          Swal.fire({
            position: "top-end",
            icon: "success",
            title: "Assignment updated suceesfully!",
...
<div className="">
  <div className="  px-6 py-12 bg-white border-0 shadow-lg sm:rounded-3xl">
    <h1 className="text-2xl font-bold mb-8">Edit Assignment</h1>
